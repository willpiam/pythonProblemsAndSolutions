...
class person(object):
    ...
    def update_file(self):#updates file holding charecters details
...
BACK SLOT: """+self.back_slot+"""
BELT SLOT: """+self.belt_slot+"""
...
def build_from_file(fileName):#program rebuilds a charecter from a file
    ...
    char = person(name, typ, race, strength, intel, wise, chaisma, pack, back, belt, r, l)
    return char

def menu():#main menu for user to see... returns True when the user wishes to quit the program
    print ("""

WHAT WOULD YOU LIKE TO DO?

A.       CREATE A NEW CHARECTER

B.       DISPLAY A CHARECTER

C.       QUIT THE PROGRAM

ENTER YOUR CHOICE BY TYPING THE LETTER CORISPONDING TO WHAT YOU WISH TO DO.
""")
    DO = input(">>")
    DO = DO.upper()
    if (DO == "C"):
        print ("Goodbye!")
        return True#ends the loop found in main and ends the program
    elif (DO == "A"):
        print ("Okay lets make a new charecter!")
        name = get_first_name()
        ...
        r = ran_thing()
        l = ran_thing()
        player = person(name, typ, race, strength, intel, wise, charisma, pack, back, belt, r, l)
        player.update_file()#puts the charecter into a txt file
        print ("CHARECTER NAMED "+name.upper()+" HAS BEEN MADE")
    elif (DO == "B"):
        print ("""
PLEASE ENTER THE NAME OF A FILE HOLDING THE DETAILS OF THE CHARECTER YOU WISH TO DISPLAY.
THESE FILES ARE NAMED IN THIS FORMAT: firstname secondname.txt
ENTERING A FILE WHICH DOES NOT EXCIST COULD CRASH THIS PROGRAM.
""")
        try:
            fileName = input(">>")
            player = build_from_file(fileName)
            print (player)
        except:
            print ("YOU ENTERED A FILE WHICH DOES NOT EXCIST.  PLEASE DON'T DO THAT AGAIN")
            time.sleep(2)#waits for a few seconds to allow the reader a chance to notice the above line before more text is written
    else:
        print ("THE INPUT YOU GAVE IS NOT A VALID OPTION. PLEASE READ INSTRUCTIONS AS THEY ARE PROVIDED.")
        time.sleep(2)
    return False#must be the very last line in this function
...
def main():
    ...
    player2 = build_from_file("e e.txt")
    print (player2)
    print (player1)
    player1.update_file()"""
